...
const CountGraph = (props) => {
  const { groupedTransactions, KaiduCount } = props;
  const [state, setState] = useState({});

  useEffect(() => {
    const transactionsData = groupedTransactions;
    const kaiduTrafficCount = KaiduCount;
    ...
    transactionsData.map((transaction) => {
      ...
      for (i; i < labelsData.length; i++) {
        if (labelsData[i] === format(new Date(transaction.date), "HH:00")) {
          valuesData[i] = transaction.count;
        }
      }
    });
    ...
    kaiduTrafficCount.map((count) => {
      for (let i = 0; i < labelsData.length; i++) {
        if (labelsData[i] === format(new Date(count.date), "HH:mm")) {
          kaiduWifiData[i] = count.wifi + count.bluetooth + count.nonapple;
        }
      }
    });
  ...
  }, [groupedTransactions, KaiduCount]);
